...
import { initializePrana, stopPrana } from "./utils/prana/pranaIntegration";
import PranaDemo from "./pages/PranaDemo";
import InactivityTracking from "./pages/InactivityTracking";
...
function AppContent() {
  ...
  // Initialize PRANA monitoring when user logs in
  useEffect(() => {
    if (user?.id) {
      console.log('[PRANA] Initializing monitoring for user:', user.name);
      initializePrana({ user_id: user.id });
      
      // Cleanup on logout or unmount
      return () => {
        console.log('[PRANA] Stopping monitoring');
        stopPrana();
      };
    }
  }, [user?.id]);
