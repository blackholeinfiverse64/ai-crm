// Standalone copy of Gurukul's PRANA signal capture layer
...
class SignalCapture {
  constructor() {
    ...
    this.signals = {
      ...
      mouse_velocity: 0,
      inactivity_ms: 0,
      keyboard_inactivity_ms: 0,
      mouse_inactivity_ms: 0,
      keypress_count: 0,
      typing_speed_wpm: 0,
      tab_visible: true,
      panel_focused: true
    ...
    this._lastMouseActivity = performance.now();
  ...
  _attachKeyboardListeners() {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      // Track keypresses for typing speed
      this._keypressTimestamps.push(now);
      
      // Keep only keypresses from last 60 seconds for WPM calculation
      this._keypressTimestamps = this._keypressTimestamps.filter(t => now - t < 60000);
      
      // Update keypress count
      this.signals.keypress_count = this._keypressTimestamps.length;
      
      // Calculate typing speed (WPM) - average word is 5 characters
      if (this._keypressTimestamps.length >= 5) {
        const timeSpanSeconds = (now - this._keypressTimestamps[0]) / 1000;
        if (timeSpanSeconds > 0) {
          const wordsTyped = this._keypressTimestamps.length / 5;
          this.signals.typing_speed_wpm = Math.round((wordsTyped / timeSpanSeconds) * 60);
        }
      } else {
        this.signals.typing_speed_wpm = 0;
      }

      this._updateActivity('keyboard');
    }, { passive: true });
  }
  ...
  _attachInactivityTracker() {
    ...
    setInterval(() => {
      ...
      this.signals.keyboard_inactivity_ms = Math.round(now - this._lastKeyboardActivity);
      this.signals.mouse_inactivity_ms = Math.round(now - this._lastMouseActivity);
  ...
  _updateActivity(type = 'general') {
    const now = performance.now();
    this._lastActivityTime = now;
    this.signals.inactivity_ms = 0;
    
    if (type === 'keyboard') {
      this._lastKeyboardActivity = now;
      this.signals.keyboard_inactivity_ms = 0;
    } else if (type === 'mouse') {
      this._lastMouseActivity = now;
      this.signals.mouse_inactivity_ms = 0;
    } else {
      // General activity updates both
      this._lastKeyboardActivity = now;
      this._lastMouseActivity = now;
      this.signals.keyboard_inactivity_ms = 0;
      this.signals.mouse_inactivity_ms = 0;
  ...
  _startLiveLogging() {
    ...
    this._logInterval = setInterval(() => {
      console.log('[PRANA-G SIGNALS]', {
        ...
        keyboard_inactivity_ms: this.signals.keyboard_inactivity_ms,
        mouse_inactivity_ms: this.signals.mouse_inactivity_ms,
        keypress_count: this.signals.keypress_count,
        typing_speed_wpm: this.signals.typing_speed_wpm,
